...
def get_time(p_time):
	time_args = [x if len(x) == 2 else '0%s' % x for x in p_time['t']]
...
def get_job_file(o_pbs, i_job, gpu):
	if not o_pbs:
		...
		if not gpu:
			job_file = '%s/%s_%s.pbs' % (abspath('..'), i_job, curTime)
		else:
			job_file = '%s/%s_%s_slurm.sh' % (abspath('..'), i_job, curTime)
	else:
		job_file = o_pbs
	return job_file


def get_work_dir(p_out_dir):
	if exists(p_out_dir):
		work_dir = abspath(p_out_dir)
	else:
		work_dir = str(Path.home())
	return work_dir


def get_email_address(path):
	if not isfile(path):
