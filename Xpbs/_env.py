...
def get_env(i_job: str, o_pbs: str, p_env: str, p_tmp: str, work_dir: str,
    ...
    if p_env:
        ...
        if p_env == 'virtualenv__mmvec':
            ...
            env.append('source virtualenv__mmvec/bin/activate"')
    ...
    if p_scratch_path and loc:
        ...
        for ff in sorted(ff_dirs):
            if ff not in copied_dirs:
                env.append('if [ -d %s ]; then cp -r --parents %s ${locdir}; fi 2>/dev/null\n' % (ff, ff))
        for ff in ff_paths:
            env.append('if [ -f %s ]; then cp -r --parents %s ${locdir}; fi 2>/dev/null\n' % (ff, ff))
